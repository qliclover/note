...
export default function Home() {
  ...
  if (loading) return <p className="p-8" style={{ color: 'var(--muted)' }}>Loading...</p>;
      ...
      {user ? (
        <div className="flex-1 flex flex-col justify-center" style={{ gap: '30px' }}>
          ...
          <div className="flex flex-col" style={{ gap: '16px' }}>
            <Link href="/dashboard" className="btn" style={{ background: 'var(--fg)', color: 'var(--bg)' }}>
              Go to Nook
            </Link>
            <button onClick={handleLogout} style={{ textAlign: 'center', fontSize: '15px', color: 'var(--muted)' }}>
              Logout
            </button>
          </div>
        </div>
        ...
        <>
          <div className="flex-1 flex flex-col justify-center" style={{ gap: '30px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '16px', color: 'var(--muted)', marginTop: '18px', lineHeight: 1.5 }}>
            ...
            <div style={{ borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', padding: '18px 0' }}>
              ...
              <div style={{ fontFamily: 'var(--font-serif), serif', fontSize: '44px', letterSpacing: '-.5px', marginTop: '2px' }}>
                $3,924<span style={{ fontStyle: 'italic', color: 'var(--muted)' }}>.90</span>
              </div>
              <div style={{ fontSize: '13px', color: 'var(--muted)', marginTop: '6px' }}>+$4,200 in · −$275.10 out</div>
            </div>
          </div>
          <div className="flex flex-col" style={{ paddingBottom: '44px', gap: '16px' }}>
            <Link href="/register" className="btn" style={{ background: 'var(--fg)', color: 'var(--bg)' }}>
              Get started
            </Link>
            <Link href="/login" style={{ textAlign: 'center', fontSize: '15px', color: 'var(--muted)' }}>
