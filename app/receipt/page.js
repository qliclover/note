...
export default function ReceiptPage() {
    ...
    return (
        <div className='max-w-md mx-auto p-6'>
            ...
            <input type='file' accept='image/*' onChange={handleFile} className='mb-4 block w-full text-sm'/>
            {image && (
                <img src={image} alt='preview' className='w-full rounded-lg border mb-4 max-h-72 object-contain'/>
            )}

            {image && (
                <button onClick={handleScan} disabled={loading}
                    className='w-full bg-blue-600 text-white rounded-lg py-2 mb-4 disabled:opacity-50'>
...
function ReceiptCard({r}) {
    ...
    return (
        <div className='bg-white shadow-lg rounded-lg p-6 font-mono text-sm'>
            <div className='text-center border-b border-dashed border-gray-300 pb-3 mb-3'>
                <p className='text-lg font-bold'>{r.merchant}</p>
                {r.date && <p className='text-gray-500'>{r.date}</p>}
            </div>
            <div className='flex flex-col gap-1 border-b border-dashed border-gray-300 pb-3 mb-3'>
                {r.items?.map((it, i) => (
            ...
            <div className='flex justify-between border-t border-dashed border-gray-400 mt-3 pt-3 font-bold text-base'>
                <span>TOTAL</span>
